...
// ============================================
const CONTRACT_ADDRESS = "0xd9145CCE52D386f254917e481eB44e9943F39138"; // PASTE DEPLOYED ADDRESS
...
// Export a promise that resolves when the contract is ready to read
window.contractReady = initContract();
...
/**
 * Get all loans (from ID 0 to loanCount-1).
 */
async function getAllLoans() {
  ensureReadContract();
  const count = await readContract.loanCount();
  console.log("Loan count:", Number(count));
  const loans = [];
 ...
 * @returns {Promise<string>} Transaction hash
 */
async function fundLoan(loanId, principalEth, borrowerAddress) {
  ensureWriteContract();
  ...
  try {
    ...
    // Fallback: If no real contract is deployed or loan ID is just a local UI fake ID, send eth directly to borrower
    if (Number(loanId) >= 9999 && borrowerAddress && borrowerAddress !== 'Unknown') {
...
// AUTO CONNECT WALLET IF ALREADY APPROVED
